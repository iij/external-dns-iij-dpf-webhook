...
**`.github/dependabot.yml` が設定するのはバージョン更新だけである。** 次は
リポジトリ設定であり、本契約の検査の対象外になる (research R8)。

| 項目 | 所在 | 対応する要件 |
|---|---|---|
| Dependabot alerts / security updates の有効化 | リポジトリ設定 (Settings) | **FR-003、SC-003** |
| ブランチ保護 (必須検査 + レビュー承認) | リポジトリ設定 | FR-009、FR-010 |

- これらを `.github/dependabot.yml` で表現しようとしないこと (MUST NOT)。
  該当する項目が存在しない。
- **セキュリティ更新が無効なままだと、`cooldown` の記述に関わらず SC-003
  (脆弱性の修正が公開から 1 日以内に提案される) は成立しない。** 設定ファイルを
  読んでも気付けないため、quickstart の確認手順で担保する。
