...
Dependabot 自身が非公開モジュールを解決するための資格情報だけは必要になる。
これはワークフローが参照しない名前の Dependabot secret として持つ。ワークフローが
参照しなければ実行環境に現れないため、依存のコードから届かない。
...
- 非公開モジュール `github.com/iij/dpf-go` の解決に資格情報が必要 (公開後は不要)
