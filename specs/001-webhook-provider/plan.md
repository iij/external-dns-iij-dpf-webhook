...
技術的な要点は 4 つ。**(1)** 変更の適用は「ゾーンロック取得 → 公開中レコードの全件取得 →
...
- **送信量**: 適用のたびにゾーン全件を送信する。1,000 件規模で実際に通るかは
  検証用ゾーンでの確認が必要 (research R3 の未確定項目)
...
- SOA とゾーン apex の NS を投入対象から外すことで、FR-029 (apex NS を削除しない) が
  自前ロジックではなく API 側の既定によって担保される
