...
| ExternalDNS への応答 | 出 | **末尾ドットを除いて返す** (下記の訂正) |
...
**訂正 (2026-09-16、004 の実環境で判明)**: 「レコード一覧の応答は正規化名のまま
返してよい」は**誤りだった**。

ExternalDNS の TXT レジストリは、所有権レコードの有無を**素の文字列一致**で照合する。

	txtRecordsSet.Insert(record.DNSName)        // provider が返した名前
	...
	if !txtRecordsSet.Has(desiredTXT.DNSName)   // 自分で生成した名前
		ep.WithProviderSpecific("txt/force-update", "true")

生成側は末尾ドットを持たない。**末尾ドット付きで返すと照合が必ず外れ、
`txt/force-update` が current に付く。** すると `plan` の
`providerSpecificChanged` が真になり (current にあって desired に無いため)、
同じ差分が毎周回再適用される (SC-007 の破れ)。

当初の「確認済み」は `plan` の突き合わせが `idna.NormalizeDNSName` で正規化される
ことに基づいていた。**そこは正しいが、この 1 箇所だけが素の比較である。**
正規化される経路を確かめただけで、全経路を確かめたことにしていた。

したがって**応答では末尾ドットを除く** (`dnsname.Name.Unqualified`)。落とすのは
末尾ドットだけで、小文字化は保つ。内部表現と DPF へ渡す値は従来どおり正規化名
(末尾ドット付き) である。**変えたのは転送形だけである。**

受信側は変わらない。上流がどの表現で送っても受信時に正規化するため、本 provider の
動作は受け取る表現に依存しない。
