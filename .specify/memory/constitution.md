...
- Bump rationale: 外部 API のエラー応答を全文記録することを規範として追加した。
  切り詰めを禁じたのは、DPF の応答が含む request_id がサポートへの
  問い合わせのキーであり、長さで切ると失われるため。
  原則 V の実質的な拡張のため MINOR。
- Modified principles:
  - V. 可観測性と運用性: 外部 API のエラー応答の扱いを追加。
...
- Bump rationale: main へのマージ前に、実際の DPF に対してレコードの追加・変更・
  削除を CI で検証することを MUST とした。あわせて同検証の並列実行を禁じた。
  規範の追加のため MINOR。
- Modified principles: なし
- Modified sections:
  - 開発ワークフローと品質ゲート: 「実環境での検証」サブセクションを新設。
- Added sections: なし
- Removed sections: なし
- Deferred TODOs: なし
...
- Bump rationale: 自前の Helm チャートを維持する要件を削除した。上流の
  external-dns チャートが webhook provider をサイドカーとして扱えるため、
  自前のチャートは重複であり、利用者に「どちらを使うか」の分岐を生む。
  MUST の削除にあたるため MAJOR。
- Modified principles:
  - VI. Default-Deny: 対象を「バイナリと Helm チャート」から
    「バイナリと、上流チャートへ与える推奨 values」へ改めた。
- Modified sections:
  - 技術・配布制約 > 配布: 自前チャートの維持要件を削除。上流チャートを
    デプロイ経路とし、推奨 values を README に示す要件へ置き換えた。
  - 技術・配布制約 > セキュリティ: コンテナイメージとチャートの既定拒否:
    「チャートの既定値」を「README が示す推奨 values」に読み替えた。
  - 技術・配布制約 > 利用者向け文書 (README): デプロイ方法の記載を追加。
  - 技術・配布制約 > Go コード品質: govulncheck の報告に対応できない場合の
    例外規定を削除。「到達可能な既知脆弱性を報告する状態でリリースしない
    (MUST NOT)」が例外なしの禁止として残る。
  - 開発ワークフローと品質ゲート: 自前チャートの lint ゲートを削除。
- Added sections: なし
- Removed sections: なし
- Deferred TODOs: なし
...
- Bump rationale: サービスの呼称を規範として確定した。正式名称と略称を定め、
  それ以外の呼び方を禁じた。あわせて本文の表記ゆれを是正した。
  規範の追加のため MINOR。
- Modified principles: なし
- Modified sections:
  - 技術・配布制約: 「サービスの呼称」サブセクションを新設。
  - 全体: 「IIJ DNSプラットフォームサービス」(DNS の後に空白) を
    正式名称「IIJ DNSプラットフォームサービス」へ統一。
- Added sections: なし
- Removed sections: なし
- Deferred TODOs: なし

Sync Impact Report (v1.13.0)
- Version change: 1.12.0 → 1.13.0
- Bump rationale: 利用者向け文書 (README) に必ず含める内容を規範として追加した。
  サポート対象外である旨の注記と、ライセンスの明示を MUST とした。
  規範の追加のため MINOR。
- Modified principles: なし
- Modified sections:
  - 技術・配布制約: 「利用者向け文書 (README)」サブセクションを新設。
- Added sections: なし
- Removed sections: なし
- Deferred TODOs: なし
...
- Bump rationale: シークレットの混入検査を規範として追加した。main へのマージ前に
  Betterleaks で検査することを MUST とした。規範の追加のため MINOR。
- Modified principles: なし
- Modified sections:
  - 技術・配布制約: 「シークレットの検査」サブセクションを新設。
  - 開発ワークフローと品質ゲート: シークレット検査をマージの条件に追加。
  - Governance: Last Amended を実際の改訂日に更新 (v1.2.0 以降、更新漏れがあった)。
- Added sections: なし
- Removed sections: なし
- Deferred TODOs: なし
...
- Bump rationale: 依存モジュールに許容するライセンスの範囲を確定し、
  TODO(DEPENDENCY_LICENSE_POLICY) を解消した。規範の追加のため MINOR。
- Modified principles: なし
- Modified sections:
  - 技術・配布制約 > ライセンス: 許容ライセンスの明示、禁止ライセンスとその理由、
    MPL-2.0 の扱いを追加。
  - 開発ワークフローと品質ゲート: 依存ライセンスの検査を CI ゲートに追加。
- Added sections: なし
- Removed sections: なし
- Deferred TODOs: なし (本文の TODO はすべて解消)
...
- Modified principles: なし
- Modified sections:
  - 技術・配布制約: 「DPF API クライアント」サブセクションを新設。
  - 技術・配布制約 > セキュリティ: バイナリの既定拒否: トークン取得経路の限定、
    環境変数・引数からの受け取り禁止、ローテーション追随、取得失敗の扱いを追加。
  - 技術・配布制約 > セキュリティ: コンテナイメージとチャートの既定拒否:
    シークレット管理サービス参照を認証情報の受け取り経路に追加し、その egress を opt-in 化。
- Added sections: なし
- Removed sections: なし
- Deferred TODOs: なし
...
本プロジェクトは、Kubernetes の ExternalDNS に対して IIJ DNSプラットフォームサービス (DPF) を
...
- **外部 API がエラーを返した場合、その応答を全文記録すること (MUST)。**
  長さで切り詰めないこと (MUST NOT)。要約や抜粋で置き換えないこと (MUST NOT)。
- 外部 API へ送った要求の内容も、原因の特定に足る範囲で記録すること (MUST)。
  何を送ったか分からないままでは、拒否された理由を追えない。
- これらは**ログとエラーに限る**。HTTP 応答の本文へ載せないこと (MUST NOT)。
  応答本文に載せると、内部状態が呼び出し側へ漏れる経路になる。
...
**外部 API のエラー応答を切り詰めない根拠**: DPF のエラー応答は `request_id` を含み、
これがサポートへの問い合わせのキーになる。長さで切ると、末尾にあるこの値が失われ、
**問い合わせる手段そのものがなくなる**。状態コードだけを残しても、何が悪かったのかは
分からない。エラー応答は本来短く、全文を残してもログを圧迫しない。
...
バイナリと、上流チャートへ与える推奨 values の双方において、明示的に許可されていない
ものはすべて拒否すること (MUST)。許可は常に opt-in であり、拒否が既定である。
...
- 依存モジュールのライセンスは、次の**許容リスト**に限ること (MUST)。
  ...
  | MPL-2.0 | ソース提供義務を満たす形で配布物へ同梱すること (MUST) |

- **GPL / AGPL / LGPL / SSPL、およびライセンスを判別できない依存を持ち込まないこと
  (MUST NOT)。** 本サービスは静的リンクしたバイナリを配布するため、これらの依存が
  入ると配布物全体に同じ条件が及び、Apache-2.0 として配布できなくなる。
  LGPL は動的リンクなら成立しうるが、`scratch` に静的リンクで配布する本サービスでは
  再リンクの手段を利用者に提供できない。
...
**依存ライセンスの実測 (2026-09-09 時点)**: Apache-2.0 57 / BSD-3-Clause 32 /
MIT 14 / MPL-2.0 10 / BSD-2-Clause 1。すべて許容リストの範囲内であり、
GPL・AGPL・LGPL はない。MPL-2.0 の 10 件はすべて HashiCorp 系であり、
Vault 連携を取り込んだ結果として入っている。MPL-2.0 はソースの提供を求めるため、
これらはライセンス本文に加えてソースが同梱される。
...
- DPF API へのアクセスには `github.com/iij/dpf-go` を用いること (MUST)。DPF API を直接
  呼び出す HTTP クライアントを自前で実装しないこと (MUST NOT)。
- 本ライブラリの利用は、原則 II が定める DPF クライアント層の内側に閉じること (MUST)。
  生成された API 型およびライブラリ固有のエラー型を、webhook ハンドラやドメインロジックへ
  漏らさないこと (MUST NOT)。
- 本ライブラリは v1.0.0 到達前であり、マイナーバージョンの更新に破壊的変更を含みうる。
  依存バージョンを `go.mod` で固定し、更新は変更点を確認したうえで行うこと (MUST)。

**根拠**: DPF API の認証、ページング、レート制御、リトライ、非同期ジョブの完了待ちは、
自前で実装すれば必ず取りこぼしが出る領域である。既存のクライアントに委ねることで、
本プロジェクトが担う範囲を ExternalDNS の契約と DPF の間の変換に集中させる。
...
- **デプロイ方法。** 上流の external-dns Helm チャート
  (<https://kubernetes-sigs.github.io/external-dns/>) を用いた導入手順を示すこと (MUST)。
  同チャートの webhook provider 向け設定に、本イメージと必要な values を
  与える形で記載すること (MUST)。
  推奨 values は「セキュリティ: コンテナイメージとチャートの既定拒否」の要件を
  満たすこと (MUST)。利用者が安全な設定を自力で組み立てずに済むようにするため。
...
- **`main` へマージする前に、シークレットの混入を検査すること (MUST)。** 検査には
  `betterleaks` を用いること (MUST)。
- 検査は**リポジトリの履歴に対して**行うこと (MUST)。作業ツリーだけを見ないこと
  (MUST NOT)。一度コミットされた値は、後の変更で消しても履歴に残り続ける。
- 検出があった場合はマージしないこと (MUST NOT)。誤検出であると判断する場合は、
  その根拠を検査設定に記述し、検査を通る状態にしてからマージすること (MUST)。
  検査を迂回して個別に判断しないこと (MUST NOT)。
- 検査に用いるバージョンを固定すること (MUST)。ツールの暗黙の更新によって、
  同一の履歴に対する判定結果が変わる状態にしないこと (MUST NOT)。
- 混入が確認された場合、当該の秘密を**失効させること (MUST)**。履歴からの削除だけで
  済ませないこと (MUST NOT)。公開範囲に入った時点で、その値は既に漏洩している。

**根拠**: 「認証情報をリポジトリに含めない」は既にセキュリティの規範として存在するが、
守られているかを人手のレビューで確かめるのは難しい。秘密は設定例やテストの
フィクスチャ、デバッグ用の一時的な変更として紛れ込み、差分の中では他の変更と
見分けがつかない。機械的に検査して初めて規範が実効を持つ。

履歴を対象とするのは、`git` の性質による。作業ツリーだけを見る検査は、
...
- **Kubernetes へのデプロイは、上流の external-dns Helm チャートを経路とする (MUST)。**
  同チャートは webhook provider をサイドカーとして扱う仕組みを備えており、
  本リポジトリで独自のチャートを維持しないこと (MUST NOT)。二つのチャートが並ぶと、
  利用者はどちらを使うべきかの判断を迫られ、設定例も二重に維持することになる。
- 上流チャートへ与える推奨 values を README に示すこと (MUST)。安全な既定を
  利用者が自力で組み立てずに済むようにするため。
...
- DPF API のアクセストークンは、マウントされたファイル、または外部のシークレット管理
  サービスから取得すること (MUST)。この 2 経路以外からトークンを受け取らないこと (MUST NOT)。
- トークンを環境変数およびコマンドライン引数から受け取らないこと (MUST NOT)。
  環境変数は Pod の情報表示やプロセスの環境から読み取れ、ファイルおよびシークレット管理
  サービスと同じ保護水準を満たさないため。
- 外部でトークンがローテーションされた場合、再起動なしに新しいトークンが使われること (MUST)。
- トークンの取得に失敗した場合は恒久的な失敗として扱い、再試行を繰り返さないこと (MUST)。
  取得できない状態は同じ要求を繰り返しても解消しないため。
- トークンの値、およびトークンを含むファイルの内容を、エラーメッセージに含めないこと
  (MUST NOT)。

**セキュリティ: コンテナイメージとチャートの既定拒否**

原則 VI を配布成果物に適用した具体要件。README が示す推奨 values は、
利用者がそれをそのまま使ったときに以下を満たすこと (MUST)。
...
- 認証情報は、Secret のマウント、または外部シークレット管理サービスの参照としてのみ
  受け取ること (MUST)。認証情報をリポジトリ、イメージ、および推奨 values に
  含めないこと (MUST NOT)。平文の値を values に書かせる例を示さないこと
  (MUST NOT)。トークンを環境変数として Pod に渡す例を示さないこと (MUST NOT)。
- シークレット管理サービスへの egress は、その利用が設定された場合にのみ許可すること
  (MUST)。利用しない構成の NetworkPolicy に、既定で穴を開けないこと (MUST NOT)。
...
- 設定項目または推奨 values を追加する PR は、その既定が拒否側であることをレビューで
  確認すること (MUST)。既定を許可側に変更する PR は、理由を記載すること (MUST)。
...
- CI は以下をすべて通過することをマージの条件とすること (MUST):
  - シークレットの混入検査 (`betterleaks` による履歴の走査)
...
モックに対するテストは、変換と分岐の正しさを示すが、DPF が実際に何を受け付けるかは
示さない。API スキーマの解釈違い、制約の見落とし、非同期ジョブの挙動は、
実際に叩いて初めて分かる。

- **`main` へマージする前に、実際の DPF に対してレコードの追加・変更・削除を
  CI で実行し、成功することを確認すること (MUST)。** 手元での確認をもって
  代えないこと (MUST NOT)。手元の環境は再現性がなく、他の変更者が同じ確認を
  繰り返せない。
- 検証には破壊的操作を行ってよい**検証用ゾーン**を用いること (MUST)。
  本番ゾーンに対して実行しないこと (MUST NOT)。
- 検証は少なくとも次を含むこと (MUST)。
  - レコードの**追加**が反映されること
  - レコードの**変更**が反映されること
  - レコードの**削除**が反映されること
  - 同一の変更を再適用しても最終状態が変わらないこと (FR-010)
- **この検証を並列に実行しないこと (MUST NOT)。** 同時に走らせる場合、
  後続の実行を待たせること (MUST)。

**根拠**: 並列実行を禁じるのは、検証用ゾーンが共有される状態だからである。
2 つの実行が同時に走ると、それぞれがゾーンを読み、マージし、書き戻す。これは
利用前提条件 PC-001 が禁じる「ロックに参加しない機械的な変更者」が 2 つ並ぶ
状況にほかならず、互いの変更を取り消し合う。

ゾーンロックによって書き込み自体は直列化されるが、それでは足りない。検証は
ゾーンの内容に対して表明を行うため、一方の実行が作ったレコードが他方の
表明に混ざる。判定が実行のタイミングに依存し、失敗が再現しなくなる。
...
**Version**: 2.2.0 | **Ratified**: 2026-09-04 | **Last Amended**: 2026-09-09
