...
- 認証情報は、Secret のマウント、または外部シークレット管理サービスの参照としてのみ
  受け取ること (MUST)。認証情報をリポジトリ、イメージ、および Helm チャートの既定値に
  含めないこと (MUST NOT)。平文の値を `values.yaml` に書かせる設定項目を用意しないこと
  (MUST NOT)。トークンを環境変数として Pod に渡す values を用意しないこと (MUST NOT)。
...
**Version**: 1.14.0 | **Ratified**: 2026-09-04 | **Last Amended**: 2026-09-09
