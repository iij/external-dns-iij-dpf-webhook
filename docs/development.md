...
**未設定の場合、`e2e` は失敗する。** 警告だけを出して通すと、実環境での検証が
**行われていない**まま緑になる。「テストが緑」と「検証が行われた」は別のことで
ある。要約 (`$GITHUB_STEP_SUMMARY`) に、何を設定すればよいかを出す。
...
**検証用ゾーンを使う検証はすべてこの 1 実行に入れる。** 規模の検証
(`scale`) とサイドカー構成の検証 (`sidecar`、`e2e-sidecar.yml` を
`workflow_call` で呼ぶ) は、`needs` で順に繋いである。別のワークフローとして
起動すると、同じ `concurrency` グループを 2 つの実行が奪い合う。GitHub は
「実行中 1 + 待機 1」しか保持せず、3 つ目が来ると**待機中のものを cancel
する**ため、push が続く間、片方が一度も走らないまま終わりうる。**cancel された
実行は検査として Pull Request に出ないため、緑に見える。**
...
| `sidecar_test.go` | 反映の待ち合わせと後始末 (SC-001) | `sidecar` (e2e の中) |
