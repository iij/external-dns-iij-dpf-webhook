...
**ゾーンロックの取得・解放は個別には計測されていない。** `dpf_api_calls_total`
には専用の `operation` として現れず、所要時間は `apply` に含まれる。ロックの
競合はログから読む ([既知の制限](#既知の制限))。
...
| `apply` | 変更の適用。ロック取得・レコードの読み直し・一括更新・反映の待ち合わせまでを 1 つとして数える |
| `list_zone_histories` | ゾーン反映履歴の取得。**受け入れ確認にのみ用い、通常の動作では呼ばない** |
| `list_records_by_label` | 管理者の印で絞ったレコードの取得。**受け入れ確認にのみ用い、通常の動作では呼ばない** |
...
| `TXT` | character-string 1 つは 255 オクテットまで。**超える場合は自動分割される**。合計に上限はない。**RDATA は引用符付きで保存される** |
